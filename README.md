...
**Проблема 6: Новые заказы не попадали в Supabase автоматически**
Крон-джоб проверял только Supabase — но синк `RetailCRM → Supabase` запускался только вручную (`python3 scripts/sync_to_supabase.py`). Новый заказ создан в RetailCRM → в Supabase его нет → крон его не видит → уведомление не приходит.

*Решение:* Встроил синк прямо в крон-маршрут `/api/cron`. Теперь при каждом запуске (каждую минуту) крон:
1. Делает `upsert` всех заказов из RetailCRM в Supabase
2. Проверяет записи где `total > 50 000` и `telegram_sent = false`
3. Отправляет уведомления и помечает `telegram_sent = true`
